...
def create_employee(
    employee_data: employee_model.EmployeeCreate, db: Session = Depends(get_db)
) -> Employee:
    manager = (
        db.query(Employee)
        .filter(Employee.id == uuid.UUID(employee_data.manager_id))
        .first()
    )
    company = (
        db.query(Company)
        .filter(Company.id == uuid.UUID(employee_data.company_id))
        .first()
    )
...
def register_account(
    employee_data: api_schemas.CreateNewAccountForm, db: Session = Depends(get_db)
):
    company_id = uuid.uuid4()

    company = Company(
        id=company_id, name=employee_data.company_name, founding_member=None
    )
    employee = Employee(
        name=employee_data.name,
        email=employee_data.email,
...
def load_employee(
    ...
    raise ValueError(
        "Invalid function call: either employee_id or email must be provided"
    )


def create_password(form_data: dict, db: Session = Depends(get_db)):
    user: Employee = load_employee(employee_id=form_data["id"], db=db)
